...
export function assessRequest(input: unknown, context: AssessmentContext = {}): PricingAssessment {
  ...
  if (input.intent === "funding" || input.intent === "finance") {
    ...
    return finish("not_supported", funding ? "Funding uses the separate assess_funding_pathways capability and its state, applicant and project fields. This pricing contract does not establish funding eligibility." : "Lender matching, borrowing authority and finance approval are not implemented in this capability. The existing finance tool is preparation guidance only.", { sources: [publicSource(funding ? SOURCE_RECORDS.funding_preparation : SOURCE_RECORDS.finance_preparation, now)], next_actions: [{ type: "view_page", label: funding ? "Funding pathway checker" : "Finance preparation", path: withProjectState(funding ? "/tools/funding-check" : "/tools/finance-check", payload.project_state ?? undefined) }], limitations: ["No eligibility, funding amount, credit approval or borrowing authority has been determined.", "No enquiry has been submitted and no provider has been contacted."] });
  }
