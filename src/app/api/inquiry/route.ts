...
const HELP_LABELS: Record<string, string> = {
  ...
  finance_options: "Finance / leasing question",
...
const MODE_LABELS: Record<string, string> = {
  ...
  finance_help: "Finance / leasing help",
...
export async function POST(req: Request) {
  try {
    ...
    const body = [
      ...
      "",
      "=== SiteComms ROUTING ===",
      "SiteComms review required — do not forward this enquiry.",
      "",
      ...
      "Where useful, ask the customer to mention SiteComms when they make contact.",
    ...
    const sendResult = await resend.emails.send({
      ...
      to: [toEmail],
      replyTo: email,
      subject: `SiteComms enquiry: ${modeLabel} - ${name}`,
      text: body,
      attachments,
    });
