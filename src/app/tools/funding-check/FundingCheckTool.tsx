...
const sections: { key: PathwayAssessment["section"]; title: string; note: string }[] = [
  { key: "investigate", title: "Routes worth investigating", note: "The basic scope is relevant to this limited catalogue. The authority must still decide applicant eligibility, costs, approval and any award." },
  { key: "check_first", title: "Clarify these conditions first", note: "Missing information, a restricted application route or an overdue review prevents a stronger conclusion." },
  { key: "watchlist", title: "Closed rounds, programme monitoring and loan signposts", note: "These are not grants currently available to you. Loans are repayable and will need a separate finance assessment." },
  ...
  return <div id="funding-tool" className="scroll-mt-24">
    <form onSubmit={submit} className="sc-card bg-white p-5 sm:p-8">
      ...
      <div className="mt-6 grid gap-6 sm:grid-cols-2">
        {step === 0 && <><ProjectStateSelector required name="funding-state" label="Project state or territory" />{select("site_type", true)}</>}
    ...
    {assessment && <section className="mt-10" aria-labelledby="funding-result-heading">
      <h2 id="funding-result-heading" ref={resultHeading} tabIndex={-1} className="text-2xl font-bold text-[var(--sc-blue-900)]">Your funding pathway review</h2>
      <div className="sc-card mt-5 bg-[var(--sc-blue-50)] p-6">
        <p className="font-semibold text-[var(--sc-blue-900)]">{assessment.summary}</p>
        <p className="mt-3 text-sm leading-relaxed text-[var(--sc-slate)]">{assessment.result.coverage_note}</p>
        <p className="mt-3 text-sm text-[var(--sc-slate)]">{projectStateName(state)} · {fieldLabel("site_type", answers.site_type ?? "")} · {fieldLabel("applicant_type", answers.applicant_type ?? "")}</p>
      ...
      {sections.map(section => {
        ...
        return section.key === "not_a_match" ? <details key={section.key} className="mt-8 rounded-xl border border-[var(--sc-border)] p-5"><summary className="cursor-pointer text-lg font-bold text-[var(--sc-blue-900)]">{section.title} ({pathways.length})</summary>{contents}</details>
          : <section key={section.key} className="mt-9"><h3 className="text-xl font-bold text-[var(--sc-blue-900)]">{section.title}</h3>{contents}</section>;
      })}
      <div className="sc-card mt-8 p-6 text-sm leading-relaxed text-[var(--sc-slate)]"><h3 className="font-bold text-[var(--sc-blue-900)]">What this review does not do</h3><ul className="mt-3 list-disc space-y-2 pl-5">{assessment.limitations.map(item => <li key={item}>{item}</li>)}</ul></div>
